...
def handle_order_post_save(sender, instance, created, **kwargs):
    ...
    if old is not None and old != instance.status:
        ...
        if instance.status == Order.Status.DELIVERED:
            _fire_in_thread(_schedule_feedback_request, instance.pk)


def _schedule_feedback_request(order_pk: int):
    try:
        from django.utils import timezone
        from datetime import timedelta
        from .models import Order, FeedbackRequest
        ...
        logger.error('Failed to schedule feedback request for order #%s: %s', order_pk, exc)
