...
def auth_telegram_view(request):
    ...
    if request.method == 'POST':
        try:
            ...
            is_courier = Courier.objects.filter(telegram_id=user.telegram_id, is_active=True).exists()
...
def profile_view(request):
    ...
    if user:
        ...
        favorites = [f.product for f in user.favorites.select_related('product').prefetch_related('product__variants')]
